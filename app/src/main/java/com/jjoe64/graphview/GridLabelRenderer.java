...
public class GridLabelRenderer {
    ...
    /**
     * the reason for the labeling callback
     */
    public enum LabelFormatterReason {              // CHANGE NOTICE: include reason and index# in the callback
        /**
         * call via the old formatLabel method
         */
        UNKNOWN,

        /**
         * general label-text sizing call
         */
        SIZING,

        /**
         * label-text sizing call for minimum value
         */
        SIZING_MIN,

        /**
         * label-text sizing call for maximum value
         */
        SIZING_MAX,

        /**
         * label-text for axis label at indicated step#
         */
        AXIS_STEP,
         ...
         * label-text for axis label at indicated step#
         */
        AXIS_STEP_SECONDSCALE,

        /**
         * label-text for data point
         */
        DATA_POINT
    }

    /**
     * Interface for callback to choose specific color per label
     */
    public interface VerticalLabelsDependentColor {     // UPDATE NOTICE:  pick text color per label
    ...
    /**
     * optional callback to choose different colors for specific vertical labels
     */
    private VerticalLabelsDependentColor mVerticalLabelsDependentColor;     // UPDATE NOTICE:  pick text color per label
    ...
    /**
     * starting and ending label points
     */
    private Double mVerticalLabelsStartY = null;    // CHANGE NOTICE: explicit control of labeling
    private Double mVerticalLabelsEndY = null;
    private Double mHorizontalLabelsStartX = null;
    private Double mHorizontalLabelsEndX = null;
    ...
    protected boolean adjustVertical() {
        ...
        //double newMinY;
        double newMinY, newMaxY;    // CHANGE NOTICE: explicit control of labeling
        double exactSteps;
        ...
        if (mGraphView.getViewport().isYAxisBoundsManual()) {
            ...
            double rangeY = maxY - newMinY;
            exactSteps = rangeY / (numVerticalLabels - 1);*/
            newMinY = minY;     // CHANGE NOTICE: explicit control of labeling
            double labelMinY = minY;
            if (mVerticalLabelsStartY != null) { labelMinY = mVerticalLabelsStartY; }
            newMaxY = maxY;
            double labelMaxY = maxY;
            if (mVerticalLabelsEndY != null) { labelMaxY = mVerticalLabelsEndY; }
            double labelRangeY = labelMaxY - labelMinY;
            exactSteps = labelRangeY / (numVerticalLabels - 1);
        } else {
            // find good steps
            ...
            newMaxY = newMinY + (numVerticalLabels - 1) * exactSteps;    // CHANGE NOTICE: explicit control of labeling
        }
        ...
        int height = mGraphView.getGraphContentHeight();
        int top = mGraphView.getGraphContentTop();                  // CHANGE NOTICE: explicit control of labeling
        double diffY = maxY - minY;                                 // CHANGE NOTICE: explicit control of labeling
        double v = newMaxY;
        if (mGraphView.getViewport().isYAxisBoundsManual() && mVerticalLabelsEndY != null) { v = mVerticalLabelsEndY; }   // CHANGE NOTICE: explicit control of labeling
        //int p = mGraphView.getGraphContentTop(); // start         // CHANGE NOTICE: explicit control of labeling
        //int pixelStep = height / (numVerticalLabels - 1);         // CHANGE NOTICE: explicit control of labeling
        for (int i = 0; i < numVerticalLabels; i++) {
            double valY = v - newMinY;                              // CHANGE NOTICE: explicit control of labeling
            double ratY = valY / diffY;                             // CHANGE NOTICE: explicit control of labeling
            double y = height * ratY;                               // CHANGE NOTICE: explicit control of labeling
            int p = (top - (int)y) + height;                        // CHANGE NOTICE: explicit control of labeling
            p = p - (mLabelVerticalHeight / 2);
            mStepsVertical.put(p, v);
    ...
    protected boolean adjustHorizontal() {
        ...
        double newMinX, newMaxX;    // CHANGE NOTICE: explicit control of labeling
        double exactSteps;
        ...
        if (mGraphView.getViewport().isXAxisBoundsManual()) {
            // scaling
            /*if (mGraphView.getViewport().mScalingActive) {            // CHANGE NOTICE: explicit control of labeling
            ...
            newMinX = minX;     // CHANGE NOTICE: explicit control of labeling
            double labelMinX = minX;
            if (mHorizontalLabelsStartX != null) { labelMinX = mHorizontalLabelsStartX; }
            newMaxX = maxX;
            double labelMaxX = maxX;
            if (mHorizontalLabelsEndX != null) { labelMaxX = mHorizontalLabelsEndX; }
            double labelRangeX = labelMaxX - labelMinX;
            exactSteps = labelRangeX / (numHorizontalLabels - 1);
            ...
            newMaxX = newMinX + (numHorizontalLabels - 1) * exactSteps;
            mGraphView.getViewport().setMinX(newMinX);
            mGraphView.getViewport().setMaxX(newMaxX);
            if (mGraphView.getViewport().getXAxisBoundsStatus() == Viewport.AxisBoundsStatus.READJUST_AFTER_SCALE) {
        ...
        int width = mGraphView.getGraphContentWidth();
        int left = mGraphView.getGraphContentLeft();        // CHANGE NOTICE: explicit control of labeling

        float scrolled = 0;
        float scrolledPixels = 0;

        double diffX = maxX - minX;                         // CHANGE NOTICE: explicit control of labeling
        double v = newMinX;
        if (mGraphView.getViewport().isXAxisBoundsManual() && mHorizontalLabelsStartX != null) { v = mHorizontalLabelsStartX; }   // CHANGE NOTICE: explicit control of labeling
        //int p = mGraphView.getGraphContentLeft(); // start        // CHANGE NOTICE: explicit control of labeling
        //float pixelStep = width / (numHorizontalLabels - 1);      // CHANGE NOTICE: explicit control of labeling
        ...
        if (!Float.isNaN(mGraphView.getViewport().mScrollingReferenceX)) {
            //scrolled = mGraphView.getViewport().mScrollingReferenceX - (float) newMinX;   // CHANGE NOTICE: explicit control of labeling
            //scrolledPixels += scrolled * (pixelStep / (float) exactSteps);                // CHANGE NOTICE: explicit control of labeling
        ...
        //p += scrolledPixels;      // CHANGE NOTICE: explicit control of labeling
        //v += scrolled;            // CHANGE NOTICE: explicit control of labeling

        for (int i = 0; i < numHorizontalLabels; i++) {
            double valX = v - newMinX;                          // CHANGE NOTICE: explicit control of labeling
    ...
    protected void calcLabelVerticalSize(Canvas canvas) {
        ...
        String testLabel = mLabelFormatter.formatLabelEx(LabelFormatterReason.SIZING_MAX, 0, mGraphView.getViewport().getMaxY(false), false);   // CHANGE NOTICE: include reason of the data point in the callback (used only in callback Overrides)
        ...
        testLabel = mLabelFormatter.formatLabelEx(LabelFormatterReason.SIZING_MIN, 0, mGraphView.getViewport().getMinY(false), false);  // CHANGE NOTICE: include reason of the data point in the callback (used only in callback Overrides)
        if (testLabel == null) testLabel = "";
    ...
    protected void calcLabelVerticalSecondScaleSize(Canvas canvas) {
        ...
        String testLabel = mGraphView.mSecondScale.getLabelFormatter().formatLabelEx(LabelFormatterReason.SIZING, 0, testY, false);     // CHANGE NOTICE: include reason of the data point in the callback (used only in callback Overrides)
        Rect textBounds = new Rect();
        mPaintLabel.getTextBounds(testLabel, 0, testLabel.length(), textBounds);
    ...
    protected void calcLabelHorizontalSize(Canvas canvas) {
        ...
        String testLabel = mLabelFormatter.formatLabelEx(LabelFormatterReason.SIZING, 0, testX, true);      // CHANGE NOTICE: include reason of the data point in the callback (used only in callback Overrides)
        if (testLabel == null) {
            testLabel = "";
        }
        Rect textBounds = new Rect();
    ...
    protected void drawHorizontalSteps(Canvas canvas) {
        ...
        for (Map.Entry<Integer, Double> e : mStepsHorizontal.entrySet()) {
            ...
            if (isHorizontalLabelsVisible()) {
                ...
                if (!mGraphView.getViewport().isXAxisBoundsManual()) {        // CHANGE NOTICE: explicit control of labeling
                ...
                String label = mLabelFormatter.formatLabelEx(LabelFormatterReason.AXIS_STEP, i, e.getValue(), true);    // CHANGE NOTICE: include reason and index# in the callback
                if (label == null) {
                    label = "";
                }
    ...
    protected void drawVerticalStepsSecondScale(Canvas canvas) {
        ...
        for (Map.Entry<Integer, Double> e : mStepsVerticalSecondScale.entrySet()) {
            ...
            String[] lines = mGraphView.mSecondScale.mLabelFormatter.formatLabelEx(LabelFormatterReason.AXIS_STEP_SECONDSCALE, i, e.getValue(), false).split("\n");     // CHANGE NOTICE: include reason and index# in the callback
    ...
    protected void drawVerticalSteps(Canvas canvas) {
        ...
        for (Map.Entry<Integer, Double> e : mStepsVertical.entrySet()) {
            ...
            if (isVerticalLabelsVisible()) {
                ...
                String label = mLabelFormatter.formatLabelEx(LabelFormatterReason.AXIS_STEP, i, e.getValue(), false);       // CHANGE NOTICE: include reason and index# in the callback
                if (label == null) {
                    label = "";
                }
                if (mVerticalLabelsDependentColor != null) {        // UPDATE NOTICE:  pick text color per label
                    mPaintLabel.setColor(mVerticalLabelsDependentColor.getColor(i, e.getValue(), false));
                }
    ...
    /**
     * set a hook to choose different colors per vertical axis label
     *
     * @param verticalLabelsDependentColor  hook
     *                              null to disable
     */
    public void setVerticalLabelsDependentColor(VerticalLabelsDependentColor verticalLabelsDependentColor) {    // UPDATE NOTICE:  pick text color per label
        this.mVerticalLabelsDependentColor = verticalLabelsDependentColor;
    }
    ...
    /**
     * @return  value of the starting and ending label points; these can be null if not yet set
     */
    public Double getVerticalLabelsStartY() { return mVerticalLabelsStartY; }   // CHANGE NOTICE: explicit control of labeling
    public Double getVerticalLabelsEndY() { return mVerticalLabelsEndY; }
    public Double getHorizontalLabelsStartX() { return mHorizontalLabelsStartX; }
    public Double getHorizontalLabelsEndX() { return mHorizontalLabelsEndX; }

    /**
     * @param startY, endY, startX, endX    point at which the label will start or end showing
     */
    public void setVerticalLabelsStartY(double startY) { this.mVerticalLabelsStartY = new Double(startY); }     // CHANGE NOTICE: explicit control of labeling
    public void setVerticalLabelsEndY(double endY) { this.mVerticalLabelsEndY = new Double(endY); }
    public void setHorizontalLabelsStartX(double startX) { this.mHorizontalLabelsStartX = new Double(startX); }
    public void setHorizontalLabelsEndX(double endX) { this.mHorizontalLabelsEndX = new Double(endX); }
