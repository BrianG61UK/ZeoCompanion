...
public class StaticLabelsFormatter implements LabelFormatter {
    ...
    public String formatLabelEx(GridLabelRenderer.LabelFormatterReason reason, int index, double value, boolean isValueX) {    // CHANGE NOTICE: include reason and index# in the callback (used only in callback Overrides)
       switch (reason) {
           ...
           case SIZING_MAX:
           case SIZING_MIN:
               // return the largest sized label                // CHANGE NOTICE: improve operational results
               if (isValueX && mHorizontalLabels != null) {
                   int fInx = 0;
                   int fLen = 0;
                   for (int i = 0; i < mHorizontalLabels.length; i++) {
                       String[] lines = mHorizontalLabels[i].split("\n");
                       for (String aLine: lines) {
                           if (aLine.length() > fLen) {
                               fInx = i;
                               fLen = aLine.length();
                           }
                       }
                   }
                   return mHorizontalLabels[fInx];
               } else if (!isValueX && mVerticalLabels != null) {
                   int fInx = 0;
                   int fLen = 0;
                   for (int i = 0; i < mVerticalLabels.length; i++) {
                       String[] lines = mVerticalLabels[i].split("\n");
                       for (String aLine: lines) {
                           if (aLine.length() > fLen) {
                               fInx = i;
                               fLen = aLine.length();
           ...
           case AXIS_STEP:
           case AXIS_STEP_SECONDSCALE:                      // CHANGE NOTICE: improve operational performance
               // return the proper label per the step index#
               if (isValueX && mHorizontalLabels != null) { return mHorizontalLabels[index]; }
               else if (!isValueX && mVerticalLabels != null) { return mVerticalLabels[index]; }
               //return mDynamicLabelFormatter.formatLabel(value, isValueX);
               return mDynamicLabelFormatter.formatLabelEx(reason, index, value, isValueX);

           default:
               // index cannot be utilized; so calculate from value
               if (isValueX && mHorizontalLabels != null) {
                   double minX = mViewport.getMinX(false);
                   double maxX = mViewport.getMaxX(false);
                   GridLabelRenderer render = mGraphView.getGridLabelRenderer();
                   if (render != null) {
                       Double startX = render.getHorizontalLabelsStartX();
                       if (startX != null) { minX = startX; }
                       Double endX = render.getHorizontalLabelsEndX();
                       if (endX != null) { maxX = endX; }
                   }
                   ...
                   GridLabelRenderer render = mGraphView.getGridLabelRenderer();
                   if (render != null) {
                       Double startY = render.getVerticalLabelsStartY();
                       if (startY != null) { minY = startY; }
                       Double endY = render.getVerticalLabelsEndY();
                       if (endY != null) { maxY = endY; }
                   }
