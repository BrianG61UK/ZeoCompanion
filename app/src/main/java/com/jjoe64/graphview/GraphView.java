// Sources: http://www.android-graphview.org/
// Sources:  https://github.com/jjoe64/GraphView
/**
 * GraphView
...
/**
 * WARNING - this GraphView version is modified from the publicly available version to fit the needs of the ZeoCompanion App.
 * These modifications are generally tagged with the comment prefix:  "// CHANGE NOTICE: ..."
 * Modifications do not alter the license terms stated in the header of this file; all modifications adopt the same license as stated in the header of this file.
 * The author of these modifications makes no claims of copyright for the modifications and makes no alterations of license terms.
 *
 * Modifications include:
 *      - optional overridable control of Axis min and max values (independent of viewport min and max values),
 *      - dataset entry indexes into most callbacks
 *      - reason codes for LabelFormatter callbacks (to make better sizing choices and separate formatting for Y axis vs Y data point labels)
 *      - optional ability to "parallel scroll" and "parallel scale" a second graph in-sync with a primary graph
 *      - optional ability to choose a color per-label along the Y-axis
 *      - optional ability to specify max and min scaling points during dynamic scaling
 *      - fixes for x-axis label placement
 *      - kludged ability to create a bitmap from the GraphView instance rather than a GUI view
 */
...
public class GraphView extends View {
     ...
     * tap detector
     */
    protected TapDetector mTapDetector;
    ...
    /**
     * optional long for the calling App to utilize in callbacks
     */
    public long mParentNumber = 0;      // CHANGE NOTICE: parallel scrolling
    ...
    protected void drawTitle(Canvas canvas) {
        if (mTitle != null && mTitle.length()>0) {
            ...
            mPaintTitle.setTypeface(Typeface.create(Typeface.DEFAULT, Typeface.BOLD));  // CHANGE NOTICE: better titles
